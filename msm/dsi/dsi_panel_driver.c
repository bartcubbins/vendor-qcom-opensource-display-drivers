...
int dsi_panel_driver_post_power_off(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_driver_vreg_ctrl(&panel->power_info, "vddr", false);
	if (rc)
		pr_err("%s: failed to disable vddr, rc=%d\n", __func__, rc);
...
int dsi_panel_driver_pre_power_on(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_driver_vreg_ctrl(&panel->power_info, "vddr", true);
	if (rc) {
		pr_err("%s: failed to enable vddr, rc=%d\n", __func__, rc);
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (refresh_rate == 120u) {
		if (dsi_panel_set_hmd_mode(panel, mode) != 0) {
			pr_err("%s: failed to send HMD command, mode=%d\n",
				__func__, panel->spec_pdata->hmd_mode);
			goto hmd_error;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < spec_pdata->area_count_table_size; i++) {
		pr_debug("%u, ", spec_pdata->area_count[i]);
...
static ssize_t dsi_panel_driver_maxluminance_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	struct dsi_panel *panel = display->panel;
	return scnprintf(buf, PAGE_SIZE, "%u", (panel->hdr_props.peak_brightness/10000));
}
...
static struct device_attribute panel_attributes[] = {
	...
	__ATTR(somc_max_luminance, S_IRUSR, dsi_panel_driver_maxluminance_show, NULL),
};
