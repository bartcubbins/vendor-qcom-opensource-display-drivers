				...
				int len)
{
	...
	reslen = dsi_ctrl_cmd_transfer(ctrl->ctrl, cmds, false);
...
static int dsi_panel_set_aod_change(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (bl_lvl < AOD_MODE_THRESHOLD)
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_LOW, false);
	else
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_HIGH, false);
...
int dsi_panel_set_flm2_mode(struct dsi_panel *panel, int mode)
{
	...
	rc = (mode == 0) ? dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_FLM2_OFF, false) :
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_FLM2_ON, false);
...
int dsi_panel_set_hmd_mode(struct dsi_panel *panel, int mode)
{
	...
	rc = (mode == 0) ? dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_HMD_OFF, false) :
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_HMD_ON, false);
...
int dsi_panel_dcs_cmd_tx(struct dsi_panel *panel, enum dsi_cmd_set_type cmd)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, cmd, false);
	if (rc)
